...
{ // f= cec19_func([3.3253000e+000, -1.2835000e+000]', 1);
  // nlhs = 1 plhs[0] nrhs = 2 prhs[0-1]	
    int  m, n;
	double  *f, *x;
	if ((nrhs < 2) || (nlhs < 1))
    {
 ...
 // toolfunction area
 void cec19_test_func(double *x, double *f, int nx, int mx, int func_num = 9)
{
	int cf_num=10,i,j;
	if (ini_flag==1)
	{
